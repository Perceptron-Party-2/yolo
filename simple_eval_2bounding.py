...
model.load_state_dict(torch.load("yolo_6.pth"))
model.eval()
...
if __name__ == "__main__":
    ...
    for i in range(GRID_SIZE):
        for j in range(GRID_SIZE):
            ...
            prediction_confidence_1 , prediction_confidence_2 = prediction[i, j, 0], prediction[i, j, 5]
            confidences = torch.cat([prediction_confidence_1.unsqueeze(0), prediction_confidence_2.unsqueeze(0)], dim=0)
            confidence_maxes, best_box = torch.max(IoUs, dim=0) 
            #best_box is 0 if B1 is better and 1 if B2 is better
            prediction_bounding_box = (best_box * prediction_confidence_2 + (1 - best_box) * prediction_confidence_1)
